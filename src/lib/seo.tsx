...
/** The one social card for the whole site, generated by scripts/make-og-assets.mjs. */
 ...
 *
 * Two things Next does that bite here, both confirmed by reading rendered HTML
 * rather than assumed:
 *
 *   1. It does NOT derive `og:title` from `title`. A page setting only `title`
 *      ships the parent's og:title and og:url, so every inner page shared as
 *      the homepage.
 *   2. Declaring `openGraph` on a page REPLACES the parent's object rather than
 *      merging into it. Fixing (1) therefore dropped the layout's og:image from
 *      all 26 inner routes, which is why the image is repeated here.
 *
 * Anything shared site-wide must be restated in this helper, not just set on
 * the layout.
 */
